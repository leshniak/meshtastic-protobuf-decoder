...
const pubTopic = (topic: string) =>
  topic.replace(`${ROOT_TOPIC}/2/e`, `${ROOT_TOPIC}/2/decoder`);
...
const mqttClient = mqtt.connect(`mqtt://${HOST}`, {
  connectTimeout: 1000,
...
mqttClient.on('connect', () => {
  mqttClient.subscribe(subTopic, (err) => {
    if (err) {
      console.error(`Error subscribing to ${subTopic}: ${err}`);
    }
  });
});
